...
impl EventsService {
    pub fn new(repo: Arc<EventsRepository>) -> Self {
        EventsService { repo }
    }
    ...
    pub fn list_events(
        ...
        let events = records.into_iter().map(event_to_response).collect();
...
fn event_to_response(r: crate::events::model::EventRecord) -> EventResponse {
    EventResponse {
        id: r.id,
        title: r.title,
